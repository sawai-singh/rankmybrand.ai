...
export class BudgetManager {
  ...
  private initializeProviderLimits(): void {
    this.providerLimits = new Map([
      [SerpApiProvider.INHOUSE, {
        dailyLimit: parseInt(process.env.INHOUSE_DAILY_LIMIT || '500'),
        monthlyLimit: parseInt(process.env.INHOUSE_MONTHLY_LIMIT || '15000'),
        costPerQuery: 0 // Free - our own scraper!
      }],
  ...
  async checkAndDeductCredits(
    ...
    if (!limits) {
      logger.warn(`[BudgetManager] Unknown provider: ${provider}`);
      return false;
    }
    ...
    if (dailyUsed + credits > limits.dailyLimit) {
      logger.warn(`[BudgetManager] Daily limit reached for ${provider}: ${dailyUsed}/${limits.dailyLimit}`);
      return false;
    }

    if (monthlyUsed + credits > limits.monthlyLimit) {
      logger.warn(`[BudgetManager] Monthly limit reached for ${provider}: ${monthlyUsed}/${limits.monthlyLimit}`);
      return false;
    }
    ...
    // Log usage
    logger.info(`[BudgetManager] Deducted ${credits} credits from ${provider}. Daily: ${dailyUsed + credits}/${limits.dailyLimit}`);
    ...
    for (const provider of this.providerLimits.keys()) {
      if (provider !== SerpApiProvider.MOCK) {
        try {
          ...
          logger.error(`[BudgetManager] Failed to get quota for ${provider}:`, error);
        }
      }
    }
  ...
  async getBestProvider(queriesNeeded: number): Promise<SerpApiProvider | null> {
    const providers = [
      SerpApiProvider.INHOUSE, // Our free in-house scraper first!
      SerpApiProvider.SERPAPI,
      SerpApiProvider.VALUESERP,
      SerpApiProvider.SCALESERP
    ];
  ...
  async resetUsage(provider: SerpApiProvider): Promise<void> {
    ...
    logger.info(`[BudgetManager] Reset usage counters for ${provider}`);
  }
  ...
  setProviderLimit(
    ...
    const limits = this.providerLimits.get(provider);
    if (!limits) {
      logger.warn(`[BudgetManager] Cannot set limits for unknown provider: ${provider}`);
      return;
    }
    ...
    logger.info(`[BudgetManager] Updated limits for ${provider}: daily=${limits.dailyLimit}, monthly=${limits.monthlyLimit}`);
  }
}