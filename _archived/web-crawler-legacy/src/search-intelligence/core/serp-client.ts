...
export class SerpClient {
  ...
  constructor() {
    this.logger = new Logger('SerpClient');
  ...
  private addProvider(config: SerpApiConfig): void {
    ...
    this.rateLimiters.set(
      config.provider,
      new RateLimiter(config.rateLimit || 10)
    );
  ...
  async search(
    ...
    if (circuitBreaker.isOpen()) {
      this.logger.warn(`Circuit breaker open for ${provider}, trying fallback`);
    ...
    try {
      ...
      this.logger.error(`SERP API error with ${provider}:`, error);
  ...
  private getAvailableProvider(): SerpApiProvider {
    ...
    // Fallback to mock if all real providers are down
